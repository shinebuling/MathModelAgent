...
class LocalCodeInterpreter(BaseCodeInterpreter):
    ...
    def _pre_execute_code(self):
        init_code = (
            ...
            f"\n"
            f"# 配置matplotlib中文字体支持\n"
            f"import matplotlib.pyplot as plt\n"
            f"import matplotlib as mpl\n"
            f"import platform\n"
            f"from matplotlib import font_manager\n"
            f"\n"
            f"def setup_chinese_font():\n"
            f"    system = platform.system()\n"
            f"    if system == 'Windows':\n"
            f"        fonts = ['SimHei', 'Microsoft YaHei', 'KaiTi', 'FangSong']\n"
            f"    elif system == 'Darwin':\n"
            f"        fonts = ['Heiti SC', 'Arial Unicode MS', 'STHeiti', 'PingFang SC', 'Hiragino Sans GB']\n"
            f"    else:\n"
            f"        fonts = ['DejaVu Sans', 'WenQuanYi Micro Hei', 'Noto Sans CJK SC']\n"
            f"    \n"
            f"    font_set = False\n"
            f"    for font in fonts:\n"
            f"        try:\n"
            f"            plt.rcParams['font.sans-serif'] = [font, 'DejaVu Sans']\n"
            f"            plt.rcParams['axes.unicode_minus'] = False\n"
            f"            plt.rcParams['font.family'] = 'sans-serif'\n"
            f"            # 测试字体是否可用\n"
            f"            fig, ax = plt.subplots(figsize=(1, 1))\n"
            f"            ax.text(0.5, 0.5, '测试', fontsize=12)\n"
            f"            plt.close(fig)\n"
            f"            font_set = True\n"
            f"            print(f'成功设置中文字体: {{font}}')\n"
            f"            break\n"
            f"        except:\n"
            f"            continue\n"
            f"    \n"
            f"    if not font_set:\n"
            f"        print('警告: 无法设置中文字体，可能显示为方框')\n"
            f"    \n"
            f"    # 设置其他参数\n"
            ...
            f"        sns.set_style('whitegrid', {{'font.sans-serif': [font, 'DejaVu Sans']}})\n"
            f"        print('Seaborn中文字体配置完成')\n"
            f"    except ImportError:\n"
            f"        pass\n"
            f"\n"
            f"setup_chinese_font()\n"
            f"\n"
            f"# 创建全局字体配置重置函数，在每次绘图前调用确保字体正确\n"
            f"def ensure_chinese_font():\n"
            f"    \"\"\"确保中文字体配置生效，在每次plt.figure()或sns绘图前调用\"\"\"\n"
            f"    system = platform.system()\n"
            f"    if system == 'Windows':\n"
            f"        fonts = ['SimHei', 'Microsoft YaHei']\n"
            f"    elif system == 'Darwin':\n"
            f"        fonts = ['Heiti SC', 'Arial Unicode MS']\n"
            f"    else:\n"
            f"        fonts = ['WenQuanYi Micro Hei', 'Noto Sans CJK SC']\n"
            f"    \n"
            f"    plt.rcParams['font.sans-serif'] = fonts + ['DejaVu Sans']\n"
            f"    plt.rcParams['axes.unicode_minus'] = False\n"
            f"    plt.rcParams['font.family'] = 'sans-serif'\n"
            f"    \n"
            f"    try:\n"
            f"        import seaborn as sns\n"
            f"        sns.set_style('whitegrid', {{'font.sans-serif': fonts + ['DejaVu Sans']}})\n"
            f"    except ImportError:\n"
            f"        pass\n"
            f"\n"
            f"print('提示：如需确保中文显示正确，请在绘图前调用 ensure_chinese_font()')\n"
            f"\n"
            f"# 为3D图创建中文字体配置辅助函数\n"
            f"def get_chinese_font():\n"
            f"    \"\"\"获取可用的中文字体对象，用于3D图等特殊情况\"\"\"\n"
            f"    system = platform.system()\n"
            f"    if system == 'Windows':\n"
            f"        fonts = ['SimHei', 'Microsoft YaHei', 'KaiTi', 'FangSong']\n"
            f"    elif system == 'Darwin':\n"
            ...
            f"        fonts = ['DejaVu Sans', 'WenQuanYi Micro Hei', 'Noto Sans CJK SC']\n"
            f"    \n"
            f"    for font_name in fonts:\n"
            f"        try:\n"
            f"            font_prop = font_manager.FontProperties(family=font_name)\n"
            f"            # 尝试获取字体文件，如果成功说明字体可用\n"
            f"            if font_manager.findfont(font_prop, fallback_to_default=False) != font_manager.findfont(font_manager.FontProperties(), fallback_to_default=False):\n"
            f"                return font_prop\n"
            f"        except:\n"
            f"            continue\n"
            f"    return None\n"
            f"\n"
            f"# 设置3D图默认字体参数\n"
            f"_chinese_font = get_chinese_font()\n"
            f"if _chinese_font:\n"
            f"    # 为3D图等特殊情况预先配置字典\n"
            f"    _font_config = {{'fontproperties': _chinese_font}}\n"
            f"    print('3D图中文字体配置完成')\n"
            f"else:\n"
            f"    _font_config = {{}}\n"
            f"    print('警告：3D图中文字体配置失败')\n"
        )
